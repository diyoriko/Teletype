...
2) Работай итеративно и правь только одну секцию за один прогон, если diff ещё большой.
3) Сохрани изменения в файлы.
4) В конце кратко перечисли, что исправлено.

Важно:
- Цель: 1:1 визуальное соответствие Figma baseline.
- Не добавляй новых зависимостей без необходимости.
- Не анализируй весь лендинг целиком, если можно локализовать проблему в одной секции.
- Приоритет секций по убыванию влияния: hero, products, cases, prices, corporate, footer.
- Если diff_ratio больше 40%, начни с одной самой грубой секции и доведи именно её, не распыляясь на остальные.
- Если есть dimension_mismatch, сначала верни точную высоту страницы, потом улучшай пиксельный diff.
